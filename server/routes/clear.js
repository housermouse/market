...
router.post('/getClearList', function (req, resp) {
    ...
    // 2) 执行SQL
    connection.query(getSql(barCode, name), function (error, data) {
...
router.post('/clearProduct',(req, resp) => {
    ...
    connection.query(getSql(data.barCode,name), function (error, getData) {
        if (error) throw error;
        ...
        connection.query(sql,(err)=>{
            if(err) throw err;
            console.log(getSql(data.barCode,name));
            if(getData.length>0){
                console.log(getData)
...
router.get("/getdatabypage" , (req,resp) => {
    ...
    const sql=`SELECT *FROM t_shopping_cart LIMIT ${start},${pageSize}`
